...
USE `opa2`$$
DROP TRIGGER IF EXISTS `opa2`.`fqs_crearaceptacion_AFTER_UPDATE` $$
DELIMITER ;
USE `opa2`;

DELIMITER $$

DROP TRIGGER IF EXISTS opa2.fqs_CrearAceptacionUpdate$$
USE `opa2`$$
CREATE DEFINER=`root`@`localhost` TRIGGER `opa2`.`fqs_CrearAceptacionUpdate` AFTER UPDATE ON `fqs_crearaceptacion` FOR EACH ROW
BEGIN
