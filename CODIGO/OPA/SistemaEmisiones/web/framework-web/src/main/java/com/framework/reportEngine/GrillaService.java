...
//    private Window w;
    ...
    /**
     * Crea la consulta (tabla vaadin) a partir de los parametros recibidos,
     * donde se permite filtrar por campos = a un texto, y exportar a multiples
     ...
     * @param query
     */
    public void creaGrilla(UI app,  VerticalLayout tLayout,
...
//        panel.setScrollable(true);
        ...
        table.setPageLength(tamanoGrilla); // tama√±o por defecto de la pagina
        campo = null; // Configura titulos y tama√±os de columnas

        for (int i = 0; i < configCampos.size(); i++) {
            campo = configCampos.get(i);
            table.setColumnHeader(campo.getCampo(), campo.getTitulo());
            // logger.debug("OPA - " + "TamaÒo del campo "+ campo.getCampo());
            if (campo.getTamano() == null || campo.getTamano() <= COL_MAX) {
...
//        panel.removeAllComponents();
    ...
    public void actualizaDatos() {
        ...
        try {   	// leee datos de la BD y pobla el contenedor
            ...
            for (int i = 0; i < cols; i++) {
                ...
                if (titulo.equals("Lotes") && campo.getCampo().equals("i_ma_martillo_id_martillo_lotes")) {
                    numCampo = infoCols.get("c_nemotecnico");
                } else if (titulo.equals("Lotes") && campo.getCampo().equals("b_es_todo_o_nada")) {
                    numCampo = infoCols.get("TN");
                } else {
                    numCampo = infoCols.get(campo.getCampo());
                }
                ...
                if (!(campo.getAlineacion().equals(Alineacion.NINGUNA))) // si trae valores de alineacion por campo
                {
                    ...
                    // logger.debug("OPA - " + "campo:"+campo.getCampo()+"-#-"+numCampo+"-"+tipo+"-"+alinea);
                }
                if (alinea.equals(Alineacion.DERECHA)) {
                    table.setColumnAlignment(campo.getCampo(), Table.ALIGN_RIGHT);
                    // logger.debug("OPA - " + "campo:"+campo.getCampo()+"-Alineacion-"+Alineacion.DERECHA);
                } else if (alinea.equals(Alineacion.CENTRO)) {
                    table.setColumnAlignment(campo.getCampo(), Table.ALIGN_CENTER);
                    // logger.debug("OPA - " + "campo:"+campo.getCampo()+"-Alineacion-"+Alineacion.CENTRO);
                } else {
                    table.setColumnAlignment(campo.getCampo(), Table.ALIGN_LEFT);
                    // logger.debug("OPA - " + "campo:"+campo.getCampo()+"-Alineacion-"+Alineacion.IZQUIERDA);
                }
            }
            ...
            for (Map<String, Object> row : data) {
                ...
                for (int i = 0; i < cols; i++) // columna por columna
                {
                    campo = configCampos.get(i);
                    if (titulo.equals("Lotes") && campo.getCampo().equals("i_ma_martillo_id_martillo_lotes")) {
                        campoName = "c_nemotecnico";
                    } else if (titulo.equals("Lotes") && campo.getCampo().equals("b_es_todo_o_nada")) {
                        campoName = "TN";
                    } else {
                        campoName = campo.getCampo().trim();
                    }
                    //logger.debug("OPA - " + "Campo:"+campo.getCampo());
                    //logger.debug("OPA - " + "Tipo de campo:"+ rsm.getColumnTypeName(numCampo));
                    //logger.debug("OPA - " + "Campo : "+ campo.getCampo()+" Valor "+ resultados.getObject(campo.getCampo().trim()));
                    //String valorCampo =  resultados.getString(campo.getCampo());
                    String valorCampo = Utiles.formateaDatos2(row.get(campoName), camposTipos.get(i),
                            null, campo.getTamano(), null, campo.getPrecision());
                    
                    if (titulo.equals("GestiÛn de oferta de venta permanente") && campo.getCampo().equals("c_tdocumento_cliente_vendedor")) {
                     
                        if(valorCampo.length()==1||valorCampo.length()==2)
                             valorCampo="";
                       }
    ...
    /**
     * Invoca al motor de reportes, enviando como fuente de datos una coleccion
     * de maps (filas)
     *
     */
    ...
    // arma la coleccion que se le enviara al motor de reportes para
        ...
        while (ite.hasNext()) {
            ...
            for (int i = 0; i < configCamposEnviar.size(); i++) {
                ...
                campo.setTamano(campoOriginal.getTamano() * COL_ZOOM);    // amplia tama√±o por multiplicador para reportes a exportar
